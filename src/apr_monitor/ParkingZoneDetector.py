...
class ParkingZoneDetector:
	...
	def __init__(self, parkingWarehouse):
		self.position_near_or_within_parking_zone = False	
		self.zone_detected = None
		self.parkingWarehouse = parkingWarehouse 

	def get_last_parking_zone_detected(self):
		return self.zone_detected

	def is_position_in_any_parking_zone(self, position):

		self.zone_detected = None
		parkingZones = parkingWarehouse.get_parking_zones()
		i = 0
		is_position_within_parking_zone = False

		while(i < len(parkingZones) and is_position_within_a_parkingZone):
			is_position_within_a_parkingZone = is_position_in_parking_zone(position, parkingZones[i])
			self.zone_detected = parkingZones[i]
			i = i + 1
			
		return is_within_a_parkingZone

	def is_position_in_parking_zone(position, parkingZone):
		self.zone_detected = None
		return self.__check_if_position_is_near_or_within_parking_zone(self, position, parkingZone)
